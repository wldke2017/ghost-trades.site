...
class AIStrategyRunner {
     ...
     */
    handleSignal(type, stake, symbol) {
        ...
        this.log(`Signal Generated: ${type} $${stake} on ${symbol}`, 'success');

        // Trigger buy in the main app
        // We need to interface with the main app's trading function. 
        // Assuming global 'sendBotPurchaseWithStrategy' or similar exists.
        // For now, we'll try to use the common function if available, or emit event.

        if (typeof window.executeAIStratTrade === 'function') {
            window.executeAIStratTrade(type, stake, symbol);
        } else {
